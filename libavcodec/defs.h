...
enum AVDiscard{
     ...
     * 我们在它们之间留出一些空间用于扩展（删除一些仅用于内部的关键帧或仅删除一些 bidir 帧）。
    */
    AVDISCARD_NONE    =-16, ///< discard nothing，不丢弃任何东西
    AVDISCARD_DEFAULT =  0, ///< discard useless packets like 0 size packets in avi，丢弃无用的数据包，例如 avi 中的 0 大小数据包
    AVDISCARD_NONREF  =  8, ///< discard all non reference，丢弃所有非引用
    AVDISCARD_BIDIR   = 16, ///< discard all bidirectional frames，丢弃所有双向帧
    AVDISCARD_NONINTRA= 24, ///< discard all non intra frames，丢弃所有非帧内帧
    AVDISCARD_NONKEY  = 32, ///< discard all frames except keyframes。丢弃除关键帧之外的所有帧
