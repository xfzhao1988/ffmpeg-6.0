...
#define av_bprint_room(buf) ((buf)->size - FFMIN((buf)->len, (buf)->size))//done
#define av_bprint_is_allocated(buf) ((buf)->str != (buf)->reserved_internal_buffer)//done
...
void av_bprint_strftime(AVBPrint *buf, const char *fmt, const struct tm *tm)
{
    ...
    while (1) {
        ...
        /* strftime does not tell us how much room it would need: let us
           ...
           strftime 没有告诉我们需要多少空间：让我们用两倍的空间重试，直到缓冲区足够大
         */
        ...
        if (av_bprint_alloc(buf, room)) {
            ...
            if (room < 1024) {
                /* if strftime fails because the buffer has (almost) reached
                   ...
                   如果 strftime 由于缓冲区已（几乎）达到其最大大小而失败，那么让我
                   们在本地缓冲区中尝试； 1k 应该足以格式化任何真实的日期+时间字符串
                */
                char buf2[1024];
            ...
            if (room) {
                /* if anything else failed and the buffer is not already
                   ...
                   如果其他任何事情失败并且缓冲区尚未被截断，让我们添加一个库存字符串并强制截断
                */
