...
/**
 * 二进制最大公约数算法（Binary GCD Algorithm）是一种高效计算两个整数最大公约数的方法。
 * 它是根据 Stein 算法（也称为二进制法）发展而来的，主要思想是利用整数的二进制表示形式进行迭代计算。
 * 这个算法的关键点在于将两个整数 a 和 b 不断右移，直到其中一个为 0，然后在另一个不为 0 的数上
 * 应用一系列的加减运算和位操作，最终得到最大公约数。
 *
 * 具体步骤如下：
 *
 * 1. 如果 a 和 b 中有一个为 0，则返回另一个数的绝对值作为最大公约数。
 *
 * 2. 计算 a 和 b 的最大公约数的二进制 GCD：
 * 如果 a 和 b 均为偶数，则 gcd(a, b) = 2 * gcd(a/2, b/2)。
 * 如果 a 是偶数，b 是奇数，则 gcd(a, b) = gcd(a/2, b)。
 * 如果 a 是奇数，b 是偶数，则 gcd(a, b) = gcd(a, b/2)。
 * 如果 a 和 b 均为奇数，则 gcd(a, b) = gcd(|a-b|/2, b)，即先取绝对值差值再除以 2。
 *
 * 3. 重复步骤 2，直到 a 或 b 为 0。最终返回另一个不为 0 的数的绝对值作为最大公约数。
 *
 * 这种算法的优点在于可以快速计算较大整数的最大公约数，并且只使用了移位、加法和减法等基本运算，
 * 不需要使用除法和取模运算，因此效率较高。
*/
/**
 * Stein 算法参考链接：https://blog.csdn.net/weixin_53286472/article/details/118068532
 *                    https://mb9axv8j44.feishu.cn/docs/doccnD1tt6ml6C4HotUWSJ1ef3e
*/
//done
int64_t av_gcd(int64_t a, int64_t b) {
    ...
    if (b == 0)
        return a;
    za = ff_ctzll(a);//获取这个整数a的二进制表示中最低位的连续零的个数
    zb = ff_ctzll(b);//获取这个整数b的二进制表示中最低位的连续零的个数
    k  = FFMIN(za, zb);
    /**
     * @brief llabs 函数是用来计算 long long int 类型整数的绝对值的函数。
     * 它返回一个 long long int 类型的整数，表示参数 __x 的绝对值。绝对值是一个数的非负值，
     * 即如果参数是正数或零，则返回该参数，如果参数是负数，则返回它的相反数。
     *
     * 例如：
     *
     * llabs(5) 返回 5。
     ...
     * llabs(0) 返回 0。
     *
     * 该函数在头文件 stdlib.h 中声明，可以在标准 C 库中使用。
     *
     */
    u = llabs(a >> za);
    v = llabs(b >> zb);
