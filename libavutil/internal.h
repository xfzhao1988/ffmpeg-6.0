...
/**
 * @brief 这段代码是使用 Microsoft Visual C++ 编译器的预处理指令，用于禁用指定的编译器警告。
 * 让我们逐步解释：
 *
 * __pragma：这是 Visual C++ 编译器的一个特殊预处理指令，用于在编译时控制编译器的行为。
 *
 * warning(push)：__pragma(warning(push)) 指令将当前的警告状态推送到编译器的警告堆栈中，
 * 以便后续可以通过 warning(pop) 恢复。
 *
 * warning(disable:1478)：__pragma(warning(disable:1478)) 指令禁用了警告编号为 1478 的警告。
 * 在这里，警告编号 1478 代表了特定的编译器警告，其含义是“在标准库中使用了已弃用的函数或功能”。
 *
 * __pragma(warning(pop))：__pragma(warning(pop)) 指令用于将之前推送到警告堆栈中的警告状态恢复到原始状态，
 * 以便后续代码不受影响。
 *
 * 因此，这段代码的作用是在编译时临时禁用编译器对已弃用功能的警告，以避免编译时产生过多的警告信息
 *
 */
